...
Type* ScopeTable::lookup_field(const string &record_name, const string &field_name)
{
...
void ScopeTable::insert_array_type(const string &name, Node *node, Type *element_type, int dimension) {
    assert(current_scope >= 0);
    scopes_[static_cast<size_t>(current_scope)]->insert_array_type(name,node,element_type,dimension);
}

void ScopeTable::insert(const string &name, Kind k, Node *node, Type type) {
    assert(current_scope >= 0);
